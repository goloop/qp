...
// GetInt is a convenience function to parse an integer query parameter
...
//	}
func ParseIntSlice(u *url.URL, key string) *Result[[]int] {
	result := &Result[[]int]{Key: key, Contains: true}
	data, ok := u.Query()[key]
...
// GetIntSlice is a convenience function to parse an integer slice query
...
//	}
func GetIntSlice(u *url.URL, key string) []int {
	return ParseIntSlice(u, key).Value
}
